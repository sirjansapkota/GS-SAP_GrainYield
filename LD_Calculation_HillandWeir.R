# Calculations for expected values of r2 under drift equilibrium [Hill and Weir (1988); implemented in Remington, et al. (2001)]

##Calculate and save LD statistics from TASSEL using sliding window of 50 SNPs and maf = 0.1
ld <- read.table("SAP_mixed_LD_50_tassel_maf0.1.txt",stringsAsFactors = FALSE,header=TRUE) ## read in table with R^2 and distance calculated from TASSEL or wherever
...
## fit a non linear model using the arbitrary C value, N is the number of the genotypes that have the SNP site
