...
my.read.vcf <- function(file, special.char="##", ...) {
    # Making a search term that looks like: "##.*", tells R to find anything containing the pattern "##" followed by anything (* is wildcard)
    ...
    for (j in 5:12) {
        ...
        for (i in 1:101) {
            ...
            if (CV.fold == "V0") {
                ...
                Z <- cvf[,CV.fold]
                # sort randomly selected individuals by taxa order, and so the pheno and geno will be in the same order when subsetted
                Z <- sort(Z) 
                X1 <- X[Z,]
                ...
                P <- Y[Z,]
                # fold is determined by folds assign while making the cvf dataframe 
                cvs <- sample(fold)
                print(cvs)
                y = P[,j]
                col = names(P[j])
