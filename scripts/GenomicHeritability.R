...
# this contain RKHS function used to calculate variance for each eigenvectors and also eigenvalues
load('/Users/ssapkot/Documents/Experiments/SAP_GS_PopStr/VarianceComp/programs/RKHS.rda') 
...
for (i in 1:ncol(Y)) {
    ...
    # run for-loop to calculate within and between subpopulation variance each each eigenvector upto 20
    for (nPCs in 0:20) {
        ...
        if(nPCs>0){
            ...
            fm <- RKHS(y=y, XF=PCs, 
                     K=list(list(V=EVD$vectors, d=EVD$values, df0=5, S0=3/2)),
                     nIter=nIter, burnIn=burnIn)
        }else{
            fm <- RKHS(y=y, K=list(list(V=EVD$vectors, d=EVD$values, df0=5, S0=3/2)),
                     nIter=nIter, burnIn=burnIn)
        }
        save(fm,file='fm.rda')
        rm(fm)
...
for (i in 1:ncol(Y)) {
    ...
    plot(H2W[,2]~I(0:20), pch=15, col='red',
         xlab='Number of eigenvectors', 
         ylab='Within-group heritability',
         type='o', ylim=tmp*c(.95,1.05))
