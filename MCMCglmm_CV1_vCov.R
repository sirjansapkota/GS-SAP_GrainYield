...
## 'x' is  predicted value  and 'y' is observed phenotypic value for a given trait, and 'race' represents the racial cluster the accessions belongs to

mTrait <- MCMCglmm(cbind(y,x) ~ trait - 1, random = ~idh(trait):race, rcov=~us(trait):units, data = data, family=c("gaussian","gaussian"), verbose = FALSE)
